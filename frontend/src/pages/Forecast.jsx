...
import AttackAnalysisPipeline from '../components/AttackAnalysisPipeline';
...
export default function ForecastPage() {
  ...
  useEffect(() => {
    fetch('/api/forecast/evaluate?session_id=SESS_001')
      .then(r => r.json())
      .then(d => {
        setForecast(d);
        setLoading(false);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {loading ? (
        ...
        <>
          <AttackAnalysisPipeline forecast={forecast} />
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '1.5rem' }}>
            ...
            <div className="glass-panel" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <div style={{ padding: '1.25rem', background: 'rgba(217, 83, 79, 0.12)', borderRadius: '8px', border: '1px solid rgba(217, 83, 79, 0.35)' }}>
                ...
                <div style={{ fontSize: '1.4rem', fontWeight: '700', color: '#F07875', marginTop: '0.25rem' }}>
                  {forecast?.likely_next_stage || 'N/A'} ({((forecast?.confidence || 0) * 100).toFixed(1)}% Confidence)
                </div>
                <div style={{ fontSize: '0.85rem', color: 'var(--beige-light)', marginTop: '0.5rem' }}>
                  ATT&CK Mapping: {forecast?.att_ck_mapping || 'None'}
                </div>
              </div>

              <AttackHeatmap forecast={forecast} />
            </div>
            ...
            <div className="glass-panel" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
                <div style={{ fontSize: '3rem', fontWeight: '800', color: (forecast?.escalation_risk || 0) > 0.7 ? 'var(--status-critical)' : 'var(--status-high)' }}>
                  {((forecast?.escalation_risk || 0) * 100).toFixed(0)}%
                </div>
